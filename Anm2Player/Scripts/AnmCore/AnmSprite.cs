...
namespace Iamsleepingnow.Anm2Player
{
    ...
    public class AnmSprite : AnmLayerRuntime, IAnmAnimatable
    {
        ...
        [BoxGroup("Auto Load 自动加载"), ShowIf(EConditionOperator.And, "autoLoadOnAwake", "autoLoadPlay"), MinValue(0)]
        ...
        [BoxGroup("Auto Load 自动加载"), ShowIf("autoLoadOnAwake")]
        [SerializeField] private LIBRARY.FilePathType autoLoadFilePathType = LIBRARY.FilePathType.StreamingAssets; // Anm文件路径类型 | Anm file path type
        ...
        private bool __autoLoadFilePathTypeIsNotComponent => autoLoadFilePathType != LIBRARY.FilePathType.Component; // 是否不是Component路径 | Is itn't Component path?
        ...
        private bool __autoLoadFilePathTypeIsComponent => autoLoadFilePathType == LIBRARY.FilePathType.Component;  // 是否是Component路径 | Is it Component path?
        ...
        [InfoBox("若FindLayers开启，优先使用当前存在于子集的图层；若FindLayers关闭，则会在初始化时重新生成子集图层\nIf FindLayers is enabled, it will prioritize using layers that already exist in the subset; if disabled, the subset layers will be regenerated during init", EInfoBoxType.Normal)]
        [BoxGroup("Auto Load 自动加载"), ShowIf("autoLoadOnAwake")]
        ...
        [SerializeField] private int autoLoadAnimationIndex = -1; // 默认播放的动画名称 | Default animation name

        [Tooltip("\n默认播放的起始帧序列\nDefault playback start frame\n")]
        [BoxGroup("Auto Load 自动加载"), ShowIf("autoLoadOnAwake"), MinValue(0)]
        [SerializeField] private int autoLoadStartFrame = 0; // 默认播放的帧数 | Default play frame

        [Button("加载 Load"), ShowIf("IsAppPlaying")]
        private void __LoadSpriteSheets() {
            if (autoLoadFilePathType != LIBRARY.FilePathType.Component) {
                LoadAnmFile(autoLoadFilePathType, autoLoadFilePath, true, out _, (anm, _) => {
                    anm.InitAnimation(autoFindLayers, autoLoadAnimationIndex, true)
        ...
        public AnmSprite LoadAnmFile(LIBRARY.FilePathType pathType, string path, bool isAsync, out AnmFile file, UnityAction<AnmSprite, List<AnmSpriteSheetTexture>> onLoaded = null) {
        ...
        public AnmSprite LoadAnmFile(TextAsset anmFileAsset, List<Texture2D> anmTextures, out AnmFile file, UnityAction<AnmSprite, List<AnmSpriteSheetTexture>> onLoaded = null) {
            ...
            autoLoadFilePathType = LIBRARY.FilePathType.Component;
            AnmFile anmFile = null;
            bool aimingCache = false; // 是否命中缓存 | Hit cache or not
            AnmFile outFile = null;
            List<AnmSpriteSheetTexture> outSpriteSheets = null;
        ...
        private void LoadAnmTextures(List<AnmSpritesheet> sheets, LIBRARY.FilePathType pathType, string rootFolder, UnityAction<List<AnmSpriteSheetTexture>> onLoaded = null) {
        ...
        /// <param name="rootFolder">【anm2文件根路径】Root folder path</param>
        /// <param name="onLoaded">【图集加载完成后执行】Callback when sprite sheets loaded</param>
        private async void LoadAnmTexturesAsync(List<AnmSpritesheet> sheets, LIBRARY.FilePathType pathType, string rootFolder,
        ...
        /// <param name="initAnimationIndex">【使用哪一个序数的动画作为初始化动画(-1为默认)】Use which animation ID as init animation (-1 is default)</param>
        /// <param name="setFrameProps">【是否设置帧属性】Is set frame properties</param>
        public AnmSprite InitAnimation(bool findChildLayers = true, int initAnimationIndex = -1, bool setFrameProps = false) {
            ...
            InitLayers(findChildLayers, initAnimationIndex); // 图层初始化 | Layers init
        ...
        /// <param name="initAnimationIndex">【使用哪一个序数的动画作为初始化动画(-1为默认)】Use which animation ID as init animation (-1 is default)</param>
        private void InitLayers(bool findChildLayers = false, int initAnimationIndex = -1) {
        ...
        public void FrameUpdateEvaluate(bool updateFrameIndex) {
            ...
            if (updateFrameIndex) {
                if (!IsReversed) { // 正向播放 | Play forward
                    ...
                    else if (currentFrameIndex < currentAnimationFrameCount - 1 - 1) { // 当前于倒数第二帧时 | When before the second-to-last frame
                    ...
                    else if (currentFrameIndex > 0 + 1) { // 当后于第二帧时 | When after the second frame
                        SetCurrentFrameIndex(currentFrameIndex - 1);
                    }
                    else {
                        OnAnimationCompleted.Invoke(GetCurrentAnimationName(), GetCurrentAnimationIndex(), GetCurrentAnimationDesc().Loop); // 动画完成 | Animation completed
            ...
            for (int la = 0; la < currentLayerAnimations.Count; la++) {
                ...
                for (int sl = 0; sl < SpawnedSpriteLayers.Count; sl++) {
                    if (SpawnedSpriteLayers[sl].LayerId == currentLayerAnimations[la].LayerId) {
                        SpawnedSpriteLayers[sl].SetLayerFrameProperty(sheet, currentLayerAnimations[la].GetFrameByFrameIndex(frameIndex), currentLayerAnimations[la].Visible, updateBounds).SetMute(!currentLayerAnimations[la].Visible); // 获取图层帧数
                    }
                }
            }
            // 非图集图层帧更新
            ...
            for (int na = 0; na < currentNullAnimations.Count; na++) {
                for (int sl = 0; sl < SpawnedNullLayers.Count; sl++) {
                    if (SpawnedNullLayers[sl].LayerId == currentNullAnimations[na].NullId) {
                        SpawnedNullLayers[sl].SetFrameProperty(currentNullAnimations[na].GetFrameByFrameIndex(frameIndex)).SetMute(!currentNullAnimations[na].Visible); // 获取非图集图层帧数
                    }
                }
            }
            // 事件更新
        ...
        public AnmSprite EvaluateCurrentFrame(bool updateBounds) {
            ...
            AnmAnimation currentAnimation = CurrentAnmFile.Animations.GetAnimationByIndex(currentAnimationIndex); // 获取当前动画 | Get current animation
            if (currentAnimation == null) return this;
            SetAnimationLayers(currentAnimation, currentFrameIndex, updateBounds); // 设置动画层
            return this;
        }
        #endregion
        ...
        private void RefreshLayersVisible() {
            SpawnedSpriteLayers.ForEach(layer => { layer.IsVisible = false; }); // 刷新所有图集图层的显示状态 | Refresh all spritesheet layers' visibility
        }
        #endregion
