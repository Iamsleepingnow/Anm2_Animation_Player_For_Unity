...
namespace Iamsleepingnow.Anm2Player
{
    ...
    public static partial class LIBRARY
    {
        ...
        public static string ConcatPathByPathType(AnmFilePathType pathType, string path) {
            string outPath = pathType switch {
                AnmFilePathType.StreamingAssets => path == string.Empty ? Application.streamingAssetsPath : System.IO.Path.Combine(Application.streamingAssetsPath, path),
                AnmFilePathType.AppData => path == string.Empty ? Application.persistentDataPath : System.IO.Path.Combine(Application.persistentDataPath, path),
                AnmFilePathType.Custom => path,
                AnmFilePathType.Component => PATH_TYPE_COMPONENT_KEYWORD,
        ...
        public static Texture2D LoadImageFile(AnmFilePathType pathType, string path) {
            string fullPath = ConcatPathByPathType(pathType, path);
            // 如果是Android平台
            if (pathType == AnmFilePathType.StreamingAssets && Application.platform == RuntimePlatform.Android) {
        ...
        public static List<Texture2D> LoadImageFiles(AnmFilePathType pathType, List<string> paths) {
        ...
        public static async Task<Texture2D> LoadImageFileAsync(AnmFilePathType pathType, string path) {
            string fullPath = ConcatPathByPathType(pathType, path);
            // 如果是Android平台
            if (pathType == AnmFilePathType.StreamingAssets && Application.platform == RuntimePlatform.Android) {
        ...
        public static async Task<List<Texture2D>> LoadImageFilesAsync(AnmFilePathType pathType, List<string> paths) {
            List<Texture2D> textures = new List<Texture2D>();
