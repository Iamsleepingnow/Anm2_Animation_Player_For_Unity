...
namespace Iamsleepingnow.Anm2Player
{
    ...
    public class AnmCachePath
    {
        ...
        public LIBRARY.FilePathType pathType = LIBRARY.FilePathType.StreamingAssets;
        /// <summary>【Anm动画缓存路径】</summary>
        public string path = "";

        public AnmCachePath(LIBRARY.FilePathType _pathType, string _path) {
            pathType = _pathType;
            path = _path;
        }
    ...
    public class AnmCacheManager : AnmSingleton<AnmCacheManager>
    {
        ...
        public void PreCacheFiles(List<AnmCachePath> paths) {
            ...
            for (int p = 0; p < paths.Count; p++) {
                if (!string.IsNullOrEmpty(paths[p].path) && paths[p].pathType != LIBRARY.FilePathType.Component) {
            ...
            for (int p = 0; p < pathsTemp.Count; p++) {
                if (pathsTemp[p].pathType != LIBRARY.FilePathType.Component) { // 当路径类型为组件时，则不进行缓存
