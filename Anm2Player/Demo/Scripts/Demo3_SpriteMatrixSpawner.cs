...
namespace Iamsleepingnow.Anm2Player.Demos
{
    ...
    public class Demo3_SpriteMatrixSpawner : MonoBehaviour
    {
        ...
        [SerializeField] public AnmFrameDroppingLevels frameDroppingLevels = null; // 帧丢弃等级配置文件 | Frame dropping level config file
        [Range(0.1f, 3.0f)]
        ...
        void Start() {
            ...
            if (btn_Spawn != null) {
                btn_Spawn.onClick.AddListener(() => { StartCoroutine(Spawn()); });
            }
            ...
            for (int x = 0; x < spawnCount.x; x++) {
                ...
                for (int y = 0; y < spawnCount.y; y++) {
                    ...
                    for (int z = 0; z < spawnCount.z; z++) {
                        // 计算当前物体的帧率降级等级 | Calculate current object's frame rate drop level
                        int level = 0;
                        if (z < 5) {
                            level = 0;
                        }
                            ...
                            sprite.LoadAnmFile(anmFileRuntime,
                                ...
                                onLoaded: (anm, sheets) => {
                                    anm.InitAnimation(shaderQuality: shaderQuality)
                                        .SetCurrentAnimation(animationNames[Random.Range(0, animationNames.Count)]) // 随机播放 | Random play
                                        .SetCurrentFrameIndex(Random.Range(0, 20)) // 设置随机帧索引 | Set random frame index
                                        .SetPlayBackSpeed(animationSpeed) // 设置播放速度 | Set playback speed
                                        .SetFrameDroppingLevel(frameDroppingLevels, level) // 设置帧率降级等级 | Set frame rate drop level
                                        .Play();
                                }
                            );
